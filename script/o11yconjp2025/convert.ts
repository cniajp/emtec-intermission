// Observability Conference Tokyo 2025のセッション・スピーカー情報を生成するスクリプト
// 変換元データ: https://github.com/o11yconjp/o11ycon.jp/tree/main/src/data
...
// イベントエイリアス
const EVENT_ALIAS = 'o11yconjp2025'
...
function main() {
  // データの読み込み
  const { data, overrides } = loadDataFromFiles()
  ...
  // データの上書き
  applyOverrides(dataTalks, dataSpeakers, overrides)

  // スピーカーの追加と設定
  addSpeakersAndAssignToTalks(dataSpeakers, dataTalks)
  ...
  // データの変換
  const { tracks, trackIdMap } = convertToTracks(dataRooms)
  const { speakers, speakerIdMap } = convertToSpeakers(dataSpeakers)
  const talks = convertToTalks(dataTalks, speakers, speakerIdMap, trackIdMap)
...
/**
 * オーバーライドデータでsessionsとspeakersを上書きする
 */
function applyOverrides(
  dataTalks: sessioniseTalk[],
  dataSpeakers: sessioniseSpeaker[],
  overrides: {
    sessions: { [key: string]: OverridesTalk }
    speakers: { [key: string]: OverridesSpeaker }
    rooms: object[]
      ...
      if (speaker) {
        ...
        dataSpeakers.push({
          ...
          profilePicture: value.profilePicture
            ? `/${EVENT_ALIAS}/${value.profilePicture.replace(/\//g, '').split('.').shift()}.jpg`
            : DEFAULT_IMAGE_PATH,
          isTopSpeaker: false,
          links: value.links,
          sessions: dataTalks
...
/**
 * 追加スピーカーの追加とTalkへの話者設定を行う
 */
function addSpeakersAndAssignToTalks(
  dataSpeakers: sessioniseSpeaker[],
  dataTalks: sessioniseTalk[]
) {
  // 追加スピーカーの追加
...
/**
 * dataRoomsからTracksへの変換処理
 */
function convertToTracks(dataRooms: sessioniseRoom[]) {
  // sort順に並び替えてidを振り直す
...
/**
 * dataSpeakersからSpeakersへの変換処理
 */
function convertToSpeakers(dataSpeakers: sessioniseSpeaker[]) {
...
/**
 * dataTalksからTalksへの変換処理
 */
function convertToTalks(
  dataTalks: sessioniseTalk[],
  speakers: Speaker[],
  speakerIdMap: { [key: string]: number },
