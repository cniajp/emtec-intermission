...
export default function Index() {
  ...
  const { confDay, eventAbbr } = router.query
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <TalkMenu view={view} eventAbbr={eventAbbr as string} />
    </div>
  )
}
          ...
          <div className="flex flex-row my-5" key={i}>
            ...
            <div className={'basis-11/12 grid grid-cols-4 gap-4'}>
              {view
                ...
                .map((talk, i) => (
                  <TalkMenuItem key={i} talk={talk} eventAbbr={eventAbbr} />
                ))}
            </div>
          </div>
        )
...
function TalkMenuItem({
  ...
  return (
    <Link
      ...
      href={{
        pathname: `/${eventAbbr}/break-dk/talks/${talk.id}`,
      }}
    >
      <div>{talk.id}</div>
      <div>{talk.title}</div>
