...
| タイトル折り返し | 日本語は文節単位。語の途中で改行しない | `PhraseText`（BudouX + `<wbr>`）+ `break-keep` |
...
- **折り返し**: `wrap-break-word`だけだと日本語はどの文字間でも改行され、「ノンエンジニ／ア」のように語が割れていた。CSSの`word-break: auto-phrase`はChromium 119以降限定でOBSのブラウザソースで効く保証がないため、BudouXで分割する方式にした。
